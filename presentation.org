...
#+begin_notes
What iota is actually doing is repeatedly adding 1.

What if we abstract that functionality a bit more?
#+end_notes
...
#+begin_src c++
template<class ForwardIt, class T, class Endo>
constexpr void iterate(ForwardIt first, ForwardIt last, T init, Endo f)
{
    while (first != last) {
        *first++ = init;
